...
void FlowField::setRadiation() {
    
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            ofVec2f v = ofVec2f(ofMap(x, 0, cols, 1, -1),ofMap(y, 0, rows, 1, -1));
            flowList[y][x].set(v);
        }
    }
}

void FlowField::setSine(){
    
    float angle=0;
    
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            float distance = ofDist(x*resolution, y*resolution, ofGetWidth()/2, ofGetHeight()/2);
            angle = ofMap(distance, 0, ofGetHeight()/2, 0, 2*PI);
            ofVec2f v2 = ofVec2f( cos(angle), sin(angle) ) ;
            flowList[y][x].set(v2);
        }
    }
}
